...
#define S_FUNCTION_NAME Simulation24b_msf
#endif
...
mr_Simulation24b_RegisterSimStateChecksum ( S ) ; ssSetNumSampleTimes ( S , 3
...
mr_Simulation24b_MdlInfoRegFcn ( S , "Simulation24b" , & retVal ) ; if ( !
retVal ) return ; }
#endif
#if SS_SFCN_FOR_SIM
...
( ! ssSetDataTypeSize ( S , mdlrefDWTypeId , sizeof ( ibfgmjp2q3s ) ) )
...
mdlInitializeSampleTimes ( SimStruct * S ) { ssSetSampleTime ( S , 0 , 0.0002
) ; ssSetOffsetTime ( S , 0 , 0 ) ; ssSetSampleTime ( S , 1 , 0.001 ) ;
...
static void mdlSetupRuntimeResources ( SimStruct * S ) { ibfgmjp2q3s * dw = ( ibfgmjp2q3s * ) ssGetDWork ( S , 0 ) ; void * sysRanPtr = ( NULL ) ; int sysTid = 0 ; ssGetContextSysRanBCPtr ( S , & sysRanPtr ) ; ssGetContextSysTid ( S , & sysTid ) ; if ( sysTid == CONSTANT_TID ) { sysTid = 0 ; } akqwlccdgw ( S , slmrGetTopTidFromMdlRefChildTid ( S , 0 , false ) , slmrGetTopTidFromMdlRefChildTid ( S , 1 , false ) , slmrGetTopTidFromMdlRefChildTid ( S , 2 , true ) , & ( dw -> rtm ) , & ( dw -> rtb ) , & ( dw -> rtdw ) , sysRanPtr , sysTid , ( ( NULL ) ) , ( ( NULL ) ) , 0 , - 1 ) ; ssSetModelMappingInfoPtr ( S , & ( dw -> rtm . DataMapInfo . mmi ) ) ; if ( S -> mdlInfo -> genericFcn != ( NULL ) ) { _GenericFcn fcn = S -> mdlInfo -> genericFcn ; } }
#define MDL_START
static void mdlStart ( SimStruct * S ) { } static void mdlOutputs ( SimStruct
* S , int_T tid ) { ibfgmjp2q3s * dw = ( ibfgmjp2q3s * ) ssGetDWork ( S , 0 )
; real_T const * i_g3pitxf35x = ( real_T * ) ssGetInputPortSignal ( S , 0 ) ;
real_T * o_B_5_1 = ( real_T * ) ssGetOutputPortSignal ( S , 0 ) ; real_T *
o_B_5_2 = ( real_T * ) ssGetOutputPortSignal ( S , 1 ) ; real_T * o_B_5_3 = ( real_T * ) ssGetOutputPortSignal ( S , 2 ) ; if ( tid == PARAMETER_TUNING_TID ) { } if ( tid != CONSTANT_TID && tid != PARAMETER_TUNING_TID ) { if ( ssIsSampleHit ( S , 0 , tid ) ) { Simulation24bTID0 ( ) ; } if ( ssIsSampleHit ( S , 1 , tid ) ) { Simulation24bTID1 ( & ( dw -> rtm ) , i_g3pitxf35x , o_B_5_1 , o_B_5_2 , o_B_5_3 , & ( dw -> rtb ) , & ( dw -> rtdw ) ) ; } } }
#define MDL_UPDATE
static void mdlUpdate ( SimStruct * S , int_T tid ) { ibfgmjp2q3s * dw = ( ibfgmjp2q3s * ) ssGetDWork ( S , 0 ) ; if ( ssIsSampleHit ( S , 0 , tid ) ) { nn5qc4vcwnTID0 ( ) ; } if ( ssIsSampleHit ( S , 1 , tid ) ) { nn5qc4vcwnTID1 ( & ( dw -> rtb ) , & ( dw -> rtdw ) ) ; } return ; } static void mdlTerminate ( SimStruct * S ) { ibfgmjp2q3s * dw = ( ibfgmjp2q3s * ) ssGetDWork ( S , 0 ) ; ldoflf1jdq ( & ( dw -> rtm ) ) ; return ; }
...
mdlrefDW = mr_Simulation24b_GetDWork ( ssGetDWork ( S , 0 ) ) ;
mxSetFieldByNumber ( ss , 0 , 1 , mdlrefDW ) ; } { mxArray * data =
mr_Simulation24b_GetSimStateDisallowedBlocks ( ) ; mxSetFieldByNumber ( ss ,
...
mr_Simulation24b_SetDWork ( ssGetDWork ( S , 0 ) , mxGetFieldByNumber ( ss ,
