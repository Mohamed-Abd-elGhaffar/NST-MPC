classdef LoggerBlock < matlab.System & coder.ExternalDependency
    % LoggerBlock: Logs Omega, Load, GateS(7), AggS(9) to CSV
    
    methods (Access = protected)
        function stepImpl(~, gateS, aggS,Pg,Pa)
            if coder.target('Rtw')
                coder.cinclude('csv_logger.hpp');
                
                % Pass arrays by reference
                coder.ceval('log_full_row', ...
                    coder.ref(gateS), ...
                    coder.ref(aggS), ...
                    coder.ref(Pg), ...
                    coder.ref(Pa));
    ...
    methods (Static)
        ...
        function updateBuildInfo(buildInfo, context)
            if context.isCodeGenTarget('rtw')
                buildInfo.addSourceFiles('csv_logger.cpp');
